...
import Eff.Internal.Eff1 as E
import Eff.Internal.OpenUnion51 (Member)

import qualified Data.Iota.Eff1 as I
import qualified Data.Iota.OpenUnion51 as I (Member)
...
main = defaultMain [
  ...
  --     , bench "iota-strict" $ whnf Strict.benchMul_Iota 10000000
  --     , bench "iota-Tagged" $ whnf Tagged.benchMul_Iota 10000000
  --     , bench "mtl" $ whnf benchMul_Error               10000000
  --     , bench "eff" $ whnf benchMul_Eff                 10000000
  --   ]
  -- , bgroup "pyth"
  --     [ bench "iota" $ whnf pythr_Iota 200
  --     , bench "iota-strict" $ whnf Strict.benchMul_Iota 200
  --     , bench "iota-Tagged" $ whnf Tagged.benchMul_Iota 200
...
benchCnt_Eff n = E.run $ E.runState m n
 where
 m = do
     x <- E.get
...
benchCnt_Iota n = I.run $ I.runState m n
 where
 m = do
     x <- I.get
     if x > 0 then I.put (x-1::Int) >> m else return ()
...
benchMul_Eff n = either id id . E.run . E.runError $ m
 where
 m = foldM f 1 (be_make_list n)
...
benchMul_Iota n = either id id . I.run . I.runError $ m
  where
    m = foldM f 1 (be_make_list n)
...
mainS_MTL n = S.runState (benchS_MTL n) 0
-- bytecode
-- (10000000,2000001)
...
mainS_Eff n =
 run $ E.runState (benchS_Eff n) (0::Integer)
-- (10000000,2000001)
...
mainRS_Eff n = run $
  flip E.runReader (0::Int) $
   E.runState (benchS_Eff n) (0::Integer)
-- (10000000,2000001)
...
instance Monad m => Alternative (ContT [r] m) where
  empty = ContT $ \k -> return []
  (<|>) (ContT m1) (ContT m2) = ContT $ \k ->
    liftM2 (++) (m1 k) (m2 k)

instance Monad m => MonadPlus (ContT [r] m)

pythr_MTL n = ((runCont (pyth1 n) (\x -> [x])) :: [(Int,Int,Int)])

pythr_EFF n = ((E.run . E.makeChoiceA $ pyth1 n) :: [(Int,Int,Int)])

pythr_Iota n = ((I.run . I.makeChoiceA $ pyth1 n) :: [(Int,Int,Int)])

mainN_MTL = print . length $
            ((runCont (pyth1 300) (\x -> [x])) :: [(Int,Int,Int)])

mainN_EFF = print . length $
            ((E.run . E.makeChoiceA $ pyth1 300) :: [(Int,Int,Int)])
