...
module Data.Iota.Unified.OpenUnion51
--  (
--   Union (..),
--   Member (..),
--   KnownNat,
...
--
-- type family FindElem2 (t :: * -> *) r where
--   FindElem2 t ('(t, n) ': r) = n
--   FindElem2 t (_ ': r) = FindElem2 t r
--
-- type family FindElem3 (t :: * -> *) r where
--     FindElem3 t (t ': r) = Sing 0
--     FindElem3 t (u ': r) = Sing 1 :+ FindElem3 t r
--     FindElem3 t _  = TypeError ('Text "The type " ':<>: 'ShowType t ':<>: 'Text " is not an element in the universe provided.")
--
-- type family FindElem4 (t :: * -> *) r :: Nat where
--   FindElem4 t (t ': r) = 0
--   FindElem4 t (u ': r) = 1 :+ FindElem4 t r
--   FindElem4 t _  = TypeError ('Text "The type " ':<>: 'ShowType t ':<>: 'Text " is not an element in the universe provided.")

type family AddElem t r where
  AddElem t '[] = (t, Sing 0) ': '[]
  AddElem t ('(t, Sing n) ': r) = (t, n :+ Sing 1) ': '[]
...
data Eff r a where
  Val :: a -> Eff r a
  E   :: KnownNat n => Union n r b -> Arrs r b a -> Eff r a
...
send t = E (Union @_ @(FindElem t r) t) (tsingleton Val)
{-# RULES
...
{-# INLINE handleRelay #-}
handleRelay :: KnownNat (Length r) => (a -> Eff r w) ->
                (forall v. t v -> Arr r v w -> Eff r w) ->
                Eff (t ': r) a -> Eff r w
handleRelay ret h m = loop m
 where
  loop (Val x)  = ret x
  loop e@(E (Union v :: Union n (t ': r) v) q)  = loopE (inline $ fromInteger $ natVal' @n proxy#) e
    where
      {-# INLINEABLE loopE #-}
      loopE (0::Int) (E u q) = h (unsafeCoerce v) (qComp q (loop))
      loopE n (E (Union v :: Union n1 (t ': r) b1) _) = pred' (SNat @n1) $ E (inline $ unsafeCoerce $ Union v :: Union (n1-1) r b) (tsingleton (qComp q (inline $ loopE (n-1))))
      -- {-# INLINE loopE #-}
      -- loopE 0 (Val x) = ret x
      -- loopE 0 (E u q) = h (unsafeCoerce v) (qComp q (loopE 0))
      -- loopE n (E (Union v :: Union n1 (t ': r) b1) _) = pred' (SNat @n1) $ E (unsafeCoerce $ Union v :: Union (n1-1) r b) (tsingleton (qComp q (loopE (n-1))))
    -- let k = qComp q loop
    --  in case u of
    --       (Union v :: Union n (t ': r) v) ->
    --         case SNat @n %~ SNat @0 of
              -- Proved _ ->  Debug.trace ("float @(Length r) = " ++ show (natVal (Proxy @(Length r)))
              --                        ++ "\t returning coersion x"
              --          ) $ h (unsafeCoerce v) k
    --           _        -> Debug.trace ("float @(Length r) = " ++ show (natVal (Proxy @(Length r)))
    --                                  ++ "\t returning singleton x"
    --                    ) $ pred' (SNat @n) $ E (Union v :: Union (n - 1) r v) (tsingleton k)


{-# INLINE handleRelay' #-}
handleRelay' :: KnownNat (Length r) => String -> (a -> Eff r w) ->
                (forall v. t v -> Arr r v w -> Eff r w) ->
                Eff (t ': r) a -> Eff r w
handleRelay' str ret h m = loop m
 where
  loop (Val x)  = ret x
  loop e@(E (Union v :: Union n (t ': r) v) q)  = loopE (inline $ natVal' @n proxy#) e
    where loopE 0 (E u q) = h (unsafeCoerce v) (qComp q (loop))
          loopE n (E (Union v :: Union n1 (t ': r) b1) _) = pred' (SNat @n1) $ E (inline $ unsafeCoerce $ Union v :: Union (n1-1) r b) (tsingleton (qComp q loop))
...
-- runFoo :: Eff (t ': r) d v -> e -> Eff r v ()

{-# INLINE [2] runReader' #-}
runReader' :: forall e r v. KnownNat (FindElem (Reader e) (Reader e ': r)) => Eff (Reader e ': r) v -> e -> Eff r v
runReader' m e = loop m where
 {-# INLINE loop #-}
 loop (Val x) = return x
 loop (E u q) = let pos = natVal $ Proxy @(FindElem (Reader e) (Reader e ': r))
                 in (
                    case decomp u of
                      Right Reader -> loop $ qApp q e
                      Left (u :: Union (n - 1) r b) ->
                        pred' (SNat @n) $
                          E u (tsingleton (qComp q loop))
                           )
--

{-# INLINE [2] runReaderStr #-}
runReaderStr :: forall e r v. KnownNat (FindElem (Reader e) (Reader e ': r)) => String -> Eff (Reader e ': r) v -> e -> Eff r v
runReaderStr str m e = loop m where
 {-# INLINE loop #-}
 loop (Val x) = Debug.trace (str ++ " return") $ return x
 loop (E u q) = let pos = natVal $ Proxy @(FindElem (Reader e) (Reader e ': r))
                 in (
                    case decomp u of
                      Right Reader -> Debug.trace (str ++ " coersion") $ loop (qApp q e)
                      Left (u :: Union (n - 1) r b) -> Debug.trace (str ++ " composition") $
                        pred' (SNat @n) $
                          E u (tsingleton (qComp q loop))
                           )

-- {-# INLINE [2] runReader' #-}
-- runReader' :: Eff (Reader e ': r) v -> e -> Eff r v
-- runReader' m e = loop m where
--  {-# INLINE loop #-}
--  loop (Val x) = return x
...
runReader :: KnownNat (Length r) => Eff (Reader e ': r) w -> e -> Eff r w
runReader m e = handleRelay return (\Reader k -> k e) m

runReader'' :: KnownNat (Length r) => String -> Eff (Reader e ': r) w -> e -> Eff r w
...
-- Inferred type
t2 :: (Member (Reader Int) r, Member (Reader Float) r, Member (Reader Integer) r) => Eff r Float
-- t2 :: Eff '[Reader Int, Reader Float] Float
t2 = do
  v2 <- Debug.trace "ask Float" $ ask -- Union 0 (Reader Int), r = 1, h = 0
  v1 <- Debug.trace "ask Int"   $ ask -- Union 1 (Reader Int), r = 0, h = 1
  v3 <- Debug.trace "ask Integer"   $ ask -- Union 1 (Reader Int), r = 0, h = 1
  v4 <- Debug.trace "ask Float" $ ask -- Union 0 (Reader Int), r = 1, h = 0
...
t2' :: (Member (Reader Int) r, Member (Reader Float) r) => Eff r (Int, Int, Int, Int, Float, Float, Float, Float)
t2' = do
  ...
  return (int1, int2, int3, int4, float1, float2, float3, float4)

t2'rr = run $ runReaderStr "Float" (runReaderStr "Int" t2' (5 :: Int)) (10 :: Float)

-- t2r :: KnownNat (Length r) => Member (Reader Float) r => Eff r Float
-- t2r = runReader t2 (10::Int)
-- t2rr :: Eff r Float
t2rr = flip (runReader'' "Float") (20::Float) . flip (runReader'' "Int") (10::Int) . flip (runReader'' "Integer") (10::Integer) $
      t2
...
-}
t2rrr' = run $ runReader'' "Integer" (runReader'' "Int" (runReader'' "Float" t2 (20::Float)) (10::Int)) (10::Integer)
...
runWriter :: KnownNat (Length r) => Eff (Writer o ': r) a -> Eff r (a,[o])
...
runError :: KnownNat (Length r) => Eff (Exc e ': r) a -> Eff r (Either e a)
runError =
...
mplus' :: Member Choose r => Eff r b -> Eff r b -> Eff r b
...
--
-- The interpreter
makeChoice :: forall a r. KnownNat (Length r) => Eff (Choose ': r) a -> Eff r [a]
makeChoice =
  handleRelay (return . (:[])) (\ (Choose lst) k -> handle lst k)
 where
...
makeChoiceA0 :: (Alternative f, KnownNat (Length r)) => Eff (NdetEff ': r) a -> Eff r (f a)
makeChoiceA0 = handleRelay (return . pure) $ \m k -> case m of
    MZero -> return empty
...
makeChoiceA m = loop [] m
 where
   ...
   loop jq (E u q) = case decomp u of
     ...
     Left (u :: Union (n - 1) r b)       ->
       pred' (SNat @n) $
         E u  (tsingleton (\x -> loop jq (qApp q x)))
-- --
-- -- -- testCA :: Eff '[NdetEff] Int
...
handle_relay_s s ret h m = loop s m
  where
    ...
    loop s (E u q)  = case decomp u of
      Right x -> h s x k
      Left (u :: Union (n - 1) r b)       ->
        pred' (SNat @n) $ E u (tsingleton (k s))
...
runState (E u q) s = case decomp u of
  ...
  Left (u :: Union (n - 1) r b)       ->
    pred' (SNat @n) $ E u (tsingleton (\x -> runState (qApp q x) s))
--
-- -- -- Examples
-- --
...
--    loop s (E (u::Union n r b) q) = case prj u :: Maybe (State s b) of
...
--    loop s (E u q) = case decomp u of
--      Right (Writer o) -> k o ()
--      Left (u :: Union (n - 1) (Reader s ': r) b) ->
--        pred' (SNat @n) $ case decomp u of
--          Right Reader -> k s s
--          Left (u :: Union ((n - 1) - 1) r b) ->
--            pred' (SNat @(n - 1)) $
--              E u (tsingleton (k s))
...
-- -- runC = handleRelay (const . return $ Done)
-- --                     (\ (Yield x k) k' -> return $ Y x (k' . k))
-- --
...
-- --  where loop (Y x k) = trace (show (x::Int)) >> local (\_->x+1) (k ()) >>= loop
-- --        loop Done    = trace "Done"
-- --
-- --        -- cl, client, ay are monomorphic bindings
-- --        client = ay >> ay >> ay
...
-- -- call = loop [] where
-- --   -- loop :: Member Choose r => [Eff (Exc CutFalse ': r) a] -> Eff (Exc CutFalse ': r) a -> Eff r a
-- --   loop jq (Val x) = return x `mplus'` next jq          -- (C2)
-- --   loop jq (E u q) = case decomp u of
-- --      Right (Exc CutFalse) -> mzero'  -- drop jq (F2)
-- --      Left u -> case prj u of
-- --        Just (Choose [])  -> next jq
-- --        Just (Choose [x]) -> loop jq $ qApp q x
-- --        Just (Choose lst) -> next $ map (qApp q) lst ++ jq
-- --        Nothing           -> E u (tsingleton (qComp q (loop jq)))
-- --   next []    = mzero'
-- --   next (h:t) = loop t h
-- --
-- -- -- The signature is inferred
